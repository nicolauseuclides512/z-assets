...
class AssetCountryController extends BaseController
{
    ...
    public function getAreaById()
    {
        try {
            ...
            $areas = $this->model->getAreaById($countryId, $provinceId, $districtId, $regionId);
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function ongkirNestedList()
    {
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function ongkirAreaById()
    {
        try {
            $countryId = $this->request->get('countryId');
            $provinceId = $this->request->get('provinceId');
            ...
            return $this->json(
                ...
                [
                    'country' => !$countryId
                        ? null
                        : AssetCountry::where('id', $countryId)
                            ->select(['id', 'name'])
                            ->first(),
                    'province' => !$provinceId
                        ? null
                        : AssetProvince::where('id', $provinceId)
                            ->select(['id', 'name', 'country_id'])
                            ->first(),
                    'district' => !$districtId
                        ? null
                        : AssetDistrict::where('id', $districtId)
                            ->select(['id', 'name', 'province_id'])
                            ->first(),
                    'region' => !$regionId
                        ? null
                        : AssetRegion::where('id', $regionId)
